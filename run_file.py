...
if doc:
    ...
    # Set to track unique tag combinations
    ...
    for r in range(2, len(tags) + 1):
        for combination in itertools.combinations(tags, r):
            # Create a sorted combination name to ensure uniqueness
            tag_combination_name = "_".join(sorted(combination))  # Sort to ensure same order
            
            # Skip if this combination has already been processed
            if tag_combination_name in inserted_combinations:
                continue
            ...
            if common_serial_numbers:
                ...
                # Add to inserted combinations set
                inserted_combinations.add(tag_combination_name)
        
        print(f"Processed combinations of size {r} out of {len(tags)} tags")

print("Done processing and saved all unique tag combinations into 'tag_combinations' collection.")
